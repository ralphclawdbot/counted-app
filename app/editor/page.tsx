...
export default function EditorPage() {
  ...
  const [sheetTab, setSheetTab] = useState<'style' | 'layers'>('style');
  const [mobileDisplayWidth, setMobileDisplayWidth] = useState(180);
  const mobileCanvasRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const update = () => {
      const vw = window.innerWidth;
      const vh = window.innerHeight;
      // Phone fits in: full height minus top bar (50) + bottom bar (60) + padding (40)
      const availH = vh - 50 - 60 - 40;
      const availW = vw - 32;
      const ratio = (config.height || 2556) / (config.width || 1179);
      const fromH = Math.floor(availH / ratio);
      setMobileDisplayWidth(Math.min(fromH, availW, 260));
    };
    update();
    ...
    if (urlToken) {
      ...
      fetch(`/api/configs?token=${token}`)
        ...
        .then((data) => {
          ...
          window.history.replaceState({}, '', `/editor?token=${token}`);
        })
        .catch(() => {
  ...
  return (
    <>
      ...
      <div className="mobile-only" style={{
        ...
        {/* Add Photo — direct shortcut to upload modal */}
        <button onClick={() => setShowUploadModal(true)}
          style={{ width: 40, height: 40, background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 8, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
        ...
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#aaa" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <line x1="10" y1="5" x2="14" y2="5" strokeWidth="2.5"/>
          </svg>
        </button>
        <div style={{ flex: 1 }} />
